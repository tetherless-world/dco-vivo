...
Custom form for adding or editing news links associated with an individual. The primary page,
ManageNewsForIndividual, should forward to this page if: (a) we are adding a new page, or 
(b) an edit link in the Manage News Links view has been clicked. But right now (a) is not implemented. 


*/
public class AddEditNewsFormGenerator extends BaseEditConfigurationGenerator implements EditConfigurationGenerator {
    public static Log log = LogFactory.getLog( AddEditNewsFormGenerator.class );
    private static String formTemplate = "addEditNewsForm.ftl";
    @Override
    ...
    protected EditConfigurationVTwo setupConfig(VitroRequest vreq, HttpSession session) throws Exception{ 
	    ...
	    config.setN3Required(list( this.getN3ForNews(), N3_FOR_URLTYPE ));
	    config.setN3Optional(list( N3_FOR_ANCHOR, N3_FOR_RANK));
	    
	    config.addUrisInScope("newsProperty",     list( "http://info.deepcarbon.net/schema#instrInitiativeNews" ));
    ...
    /* ********* N3 Assertions *********** */
    static String N3_FOR_NEWS = 
        "?subject ?newsProperty ?vcard . \n"+
        "?vcard a ?vcardType . \n" +
    ...
    static String individualVcardQuery =
        ...
        "?subject <http://info.deepcarbon.net/schema#instrInitiativeNews>  ?existingVcard . \n" +
        "}";
    ...
    private static String MAX_RANK_QUERY = ""
        ...
        + "    ?subject <http://info.deepcarbon.net/schema#instrInitiativeNews> ?vcard . \n"
    ...
    private int getMaxRank(String objectUri, String subjectUri, VitroRequest vreq) {
        ...
        if (objectUri == null) { // adding new news link   
