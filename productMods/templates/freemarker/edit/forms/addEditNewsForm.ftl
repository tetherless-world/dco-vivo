...
<#-- Template for adding/editing dco:instrInitiativeNews -->
...
<#else>
        <#assign titleVerb="${i18n().add_newsLink_for}">
        <#assign submitButtonText="${i18n().add_newsLink}">
        <#assign disabledVal=""/>
</#if>
...
<form class="customForm" action ="${submitUrl}">
    ...
    <label for="label">${i18n().newsLink_name}</label>
