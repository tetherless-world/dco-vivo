...
//        if ((!this.supportEdit) && (this.editMode == 'edit' || this.editMode == 'repair')) {
    ...
    showAutocompleteSelection: function(label, uri, selectedObj) {
        ...
        var $acDiv = (this.acSelections[$(selectedObj).attr('acGroupName')]).slice(-1)[0];
        ...
        // Append a new acDiv
        var $newAcDiv = $("<div class=\"acSelection\"></div>");
        $newAcDiv.attr("acGroupName", $acDiv.attr("acGroupName"));
        $newAcDiv.attr("id", $acDiv.attr("id"));
        var $newAcDivP = $("<p class=\"inline\"></p>");
        $newAcDivP.append("<label>" + $acDiv.find("label").html() + "</label>");
        $newAcDivP.append("<span class=\"acSelectionInfo\"></span><br />");
        $newAcDivP.append("<a href=\"\" class=\"verifyMatch\"  title=\"" + $acDiv.find("a.verifyMatch").attr("title") + "\">" + $acDiv.find("a.verifyMatch").html() + "</a> or ");
        $newAcDivP.append("<a href=\"#\" class=\"changeSelection\" id=\"changeSelection\">" + $acDiv.find("a.changeSelection").html() + "</a>");
        var $newAcDivInput = $("<input class=\"acUriReceiver\" type=\"hidden\"/>");
        $newAcDivInput.attr("id", $acDiv.find("input").attr("id"));
        $newAcDivInput.attr("name", $acDiv.find("input").attr("name"));
        $newAcDivInput.attr("value", this.blankSentinel);
        $newAcDivInput.attr("flagClearLabelForExisting", "true");
        $newAcDiv.append($newAcDivP).append($newAcDivInput);
        $acDiv.after($newAcDiv);
        this.acSelections[$(selectedObj).attr('acGroupName')].push($newAcDiv)

        $(selectedObj).parent().show();
    ...
    initDefaultBlankURI:function(selectedObj) {
        ...
        var $acDiv = (this.acSelections[$(selectedObj).attr('acGroupName')]).slice(-1)[0];
        $.each($acDiv, function() {
            $(this).find("input").val(customForm.blankSentinel);
        });
    }
